...
from nameko_salesforce.api.push_topics import PushTopicsAPIClient, get_client
...
def test_get_push_topic_by_name_not_found(api_version, client, mock_salesforce_server):
...
def test_get_push_topic_by_name_cached(api_version, client, mock_salesforce_server):
    name = "ContactUpdates"
    response_data = {
        "totalSize": 1,
        "records": [{"Id": "00..A0", "Name": name, "Query": "..."}],
...
def test_get_user_id_by_name_not_found(api_version, client, mock_salesforce_server):
    response_data = {"totalSize": 0, "records": []}
    mock_salesforce_server.get(requests_mock.ANY, json=response_data)
...
def test_get_user_id_by_name(api_version, client, mock_salesforce_server):
    ...
    name = "smith"
    response_data = {"totalSize": 1, "records": [{"Id": "00..A0", "Username": name}]}
    mock_salesforce_server.get(requests_mock.ANY, json=response_data)
    ...
    request = mock_salesforce_server.request_history[0]
    assert request.path == "/services/data/v{}/query/".format(api_version)
    assert request.query == ("q=select+id+from+user+where+username+%3d+%27smith%27")


def test_get_user_id_by_name_cached(api_version, client, mock_salesforce_server):
    name = "ContactUpdates"
    response_data = {"totalSize": 1, "records": [{"Id": "00..A0", "Username": name}]}
    mock_salesforce_server.get(requests_mock.ANY, json=response_data)
...
def test_get_record_type_id_by_name_not_found(
    ...
    assert str(exc.value) == "RecordType 'Student' of 'Contact' does not exist"


def test_get_record_type_id_by_name(api_version, client, mock_salesforce_server):
    ...
    result = client.get_record_type_id_by_name(sobject_type_name, record_typ_name)
...
def test_get_record_type_id_by_name_cached(api_version, client, mock_salesforce_server):
    sobject_type_name = "Contact"
    record_typ_name = "Student"
    response_data = {"totalSize": 1, "records": [{"Id": "00..A0"}]}
    mock_salesforce_server.get(requests_mock.ANY, json=response_data)
...
class TestDeclarePushTopic:
    ...
    def get_record_type_id_by_name(self):
        with patch.object(PushTopicsAPIClient, "get_record_type_id_by_name") as method:
            yield method

    @pytest.fixture
    def get_user_id_by_name(self):
        with patch.object(PushTopicsAPIClient, "get_user_id_by_name") as method:
            yield method

    @pytest.fixture
    def get_push_topic_by_name(self):
        with patch.object(PushTopicsAPIClient, "get_push_topic_by_name") as method:
            yield method
    ...
    def write_push_topic(
        ...
        if request.param == "create":
            ...
            create_push_topic = mock_salesforce_server.post(requests_mock.ANY, json={})
            return create_push_topic
        if request.param == "update":
            get_push_topic_by_name.return_value = {"Id": "00..A0"}
            update_push_topic = mock_salesforce_server.patch(requests_mock.ANY, json={})
    ...
    @pytest.mark.parametrize(
        ...
        (("Contact", None, "Contact"), ("Contact", "Student", "ContactStudent")),
    )
    def test_push_topic_name_for_sobject(
        self,
        get_record_type_id_by_name,
        client,
        write_push_topic,
        sobject_type,
        record_type,
    ...
    @pytest.mark.parametrize(
        ...
        (
            (
                None,
                False,
                ("SELECT Id, Name, LastModifiedById, LastModifiedDate " "FROM Contact"),
            ),
            (
                "Student",
                False,
    ...
    def test_push_topic_query_for_sobject(
        ...
        client.declare_push_topic_for_sobject(
            sobject_type, record_type, exclude_current_user=exclude_current_user
        )
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            ("notify_for_operation_create", "NotifyForOperationCreate", True),
            ("notify_for_operation_update", "NotifyForOperationUpdate", True),
            ("notify_for_operation_delete", "NotifyForOperationDelete", True),
            ("notify_for_operation_undelete", "NotifyForOperationUndelete", True),
            ("notify_for_operation_create", "NotifyForOperationCreate", False),
            ("notify_for_operation_update", "NotifyForOperationUpdate", False),
            ("notify_for_operation_delete", "NotifyForOperationDelete", False),
            ("notify_for_operation_undelete", "NotifyForOperationUndelete", False),
        ),
    )
