    ...
    class MessageMaker:
        def make_handshake_request(self, **fields):
            message = {
                ...
                "minimumVersion": (config["SALESFORCE"]["BAYEUX_MINIMUM_VERSION"]),
        ...
        def make_event_delivery_message(self, **fields):
            message = {"data": [], "channel": "/topic/example", "clientId": client_id}
            message.update(**fields)
            return message
...
def notifications(message_maker):
    return [
        ...
        },
        {
            ...
            "sobject": {"Id": "006D000000KnaXjIAJ", "Name": "TicTacToe"},
        },
        {
            "event": {
                "createdDate": "2016-03-29T16:40:08.208Z",
...
def make_salesforce_server(
    ...
    def _make(responses):
        ...
        config = {"WEB_SERVER_ADDRESS": "localhost:{}".format(salesforce_server_port)}
...
@pytest.fixture
def run_services(container_factory, config, login, make_salesforce_server, waiter):
...
def test_subscribe(message_maker, notifications, run_services, tracker):
    ...
    responses = [
        ...
        [
            message_maker.make_subscribe_response(subscription="/topic/AccountUpdates"),
            message_maker.make_subscribe_response(subscription="/topic/ContactUpdates"),
        ],
        [
        ...
        [
            message_maker.make_event_delivery_message(
                channel="/topic/ContactUpdates", data=notification_one
            ),
            message_maker.make_event_delivery_message(
                channel="/topic/AccountUpdates", data=notification_two
            ),
        ],
        ...
        [
            message_maker.make_event_delivery_message(
                channel="/topic/ContactUpdates", data=notification_three
            )
        ],
    ]
    ...
    assert set(topics) == set(["/topic/AccountUpdates", "/topic/ContactUpdates"])
...
def test_handle_notification(
    ...
    assert tracker.handle_notification.call_args_list == expected_notification_handling

    get_push_topic, create_push_topic = salesforce_api_server.request_history[:2]
...
def test_handle_sobject_notification(
    ...
    class Service:
        ...
        def handle_notification(self, sobject_type, record_type, notification):
            tracker.handle_notification(sobject_type, record_type, notification)
    ...
    responses = [
        [message_maker.make_handshake_response()],
        [
            message_maker.make_subscribe_response(subscription="/topic/Contact"),
            message_maker.make_subscribe_response(subscription="/topic/ContactStudent"),
        ],
        [
            message_maker.make_connect_response(
                advice={"reconnect": Reconnection.retry.value}
        ...
        [
            message_maker.make_event_delivery_message(
                channel="/topic/ContactStudent", data=notification_one
            ),
            message_maker.make_event_delivery_message(
                channel="/topic/Contact", data=notification_two
            ),
        ],
        # no notification to deliver within server timeout
        [message_maker.make_connect_response()],
        ...
        [
            message_maker.make_event_delivery_message(
                channel="/topic/ContactStudent", data=notification_three
            )
        ],
    ]
    ...
    topics = [message.pop("subscription") for message in subscriptions[0][0]]
    assert set(topics) == set(["/topic/Contact", "/topic/ContactStudent"])
    ...
    requests = salesforce_api_server.request_history[:3]
