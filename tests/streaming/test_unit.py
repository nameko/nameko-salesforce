...
class TestStreamingClientSetup:
    ...
    def test_setup_replay_enabled(self, container, redis_uri):
        ...
        expected_error = (
            ...
            "`PUSHTOPIC_REPLAY_ENABLED` is set to `True`".format(constants.CONFIG_KEY)
        )
...
class TestStreamingClientAuthentication:
    ...
    def login(self, access_token, server_host):
        with patch("nameko_salesforce.streaming.client.SalesforceLogin") as login:
    ...
    def test_login(self, access_token, client, config, login):
        ...
        assert client.server_uri == "https://some.salesforce.server/cometd/37.0"
        assert login.call_args == call(
            session=None,
...
class TestStreamingClientReplayStorage:
    ...
    def test_set_replay_id(self, client, redis_client):
        ...
        replay_id = int(redis_client.get("salesforce:replay_id:/topic/number/one"))
        assert replay_id == 11
    ...
    def test_get_replay_id(self, client, redis_client):
        ...
        redis_client.set("salesforce:replay_id:/topic/number/one", 11)
...
class TestMessageHandler:
    ...
    def test_handle_message(self, handler, channel_name):
        ...
        assert call_kwargs["context_data"][constants.REPLAY_ID_CONTEXT_KEY] == replay_id
...
class TestSobjectNotificationHandler:
    ...
    def test_handle_message(self, make_handler):
        ...
        call_args, call_kwargs = handler.container.spawn_worker.call_args
        assert call_args == (handler, (sobject_type, record_type, message), {})
        assert (
            call_kwargs["context_data"][constants.CLIENT_ID_CONTEXT_KEY]
            == handler.client.client_id
        )
...
class TestMessageHandlers:
    ...
    @pytest.fixture(
        params=(MessageHandler, NotificationHandler, SobjectNotificationHandler)
    )
    ...
    def test_handle_result_success_replay_disabled(self, handler):
        ...
        assert handler.handle_result(replay_id, worker_ctx, result, exc_info) == (
            result,
            exc_info,
        )
    ...
    def test_handle_result_success_replay_enabled(self, handler):
        ...
        assert handler.client.set_replay_id.call_args == call(
            handler.channel_name, replay_id
        )
